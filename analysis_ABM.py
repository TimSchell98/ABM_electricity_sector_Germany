...
class ESS:
    def __init__(self, start_year=2000, end_year=2020, governmental_funding = 5, co2_emissions_coal = 0.385, energy_content_coal = 29.75,
                ...
                ,hydro_p_max = 0.7
                ,biomass_p_max = 0.55
               ,nuclear_deprication = 0.09
                ,battery_lifetime = 15
                ,hydrogen_lifetime = 25
                 ):
        """
        ...
        self.hydro_p_max = hydro_p_max
        self.biomass_p_max = biomass_p_max
        self.nuclear_deprication = nuclear_deprication
        ...
        #import fix costs, ToDo: andere daten entfernen
    ...
    def run_network(self, i, total_demand, pv_generation_profile, wind_generation_profile, load_factor_profile):
        ...
        network.add('Generator', name='Producer_1', bus='Main_bus', p_nom=self.agents["Producer 1"]["data"].loc[self.timestep-1, "Installed_power"], marginal_cost=self.agents["Producer 1"]["data"].loc[self.timestep-1, "Marginal_cost"])
        network.add('Generator', name='Producer_2', bus='Main_bus', p_nom=self.agents["Producer 2"]["data"].loc[self.timestep-1, "Installed_power"], marginal_cost=self.agents["Producer 2"]["data"].loc[self.timestep-1, "Marginal_cost"])
        network.add('Generator', name='Producer_3', bus='Main_bus', p_nom=self.agents["Producer 3"]["data"].loc[self.timestep-1, "Installed_power"], marginal_cost=self.agents["Producer 3"]["data"].loc[self.timestep-1, "Marginal_cost"])
        ...
        network.add('Generator', name='Producer_7', bus='Main_bus', p_nom=self.agents["Producer 7"]["data"].loc[self.timestep-1, "Installed_power"], p_max_pu = self.hydro_profile, marginal_cost=self.agents["Producer 7"]["data"].loc[self.timestep-1, "Marginal_cost"])
        network.add('Generator', name='Producer_8', bus='Main_bus', p_nom=self.agents["Producer 8"]["data"].loc[self.timestep-1, "Installed_power"], p_max_pu = self.biomass_profile, marginal_cost=self.agents["Producer 8"]["data"].loc[self.timestep-1, "Marginal_cost"])
        ...
        Electricity_cost = total_cost/total_generated_amount    
        
        print("Endless producer provided: ")
        print(sum(network.generators_t.p["Endless_producer"]))
        self.Fallback_generator_supply.append(sum(network.generators_t.p["Endless_producer"]))
        ...
        self.coal_profile = network.generators_t.p["Producer_1"] / self.agents["Producer 1"]["data"].loc[self.timestep-1, "Installed_power"]
        self.gas_ct_profile = network.generators_t.p["Producer_2"] / self.agents["Producer 2"]["data"].loc[self.timestep-1, "Installed_power"]
        self.gas_cc_profile = network.generators_t.p["Producer_3"] / self.agents["Producer 3"]["data"].loc[self.timestep-1, "Installed_power"]
        
        #save marginal costs list from this year to calculate profit of storages
        self.marginal_costs = network.buses_t.marginal_price["Main_bus"]
        
        #calculate profit of storages
        #pump storage
        self.pump_profit = sum(network.links_t.p0["Unload_pump"] * network.buses_t.marginal_price["Main_bus"] - network.links_t.p0["Load_pump"] * network.buses_t.marginal_price["Main_bus"])
        
        #battery storage
        self.battery_profit = sum(network.links_t.p0["Unload_battery"] * network.buses_t.marginal_price["Main_bus"] - network.links_t.p0["Load_battery"] * network.buses_t.marginal_price["Main_bus"])
        
        #hydrogen storage
        self.hydrogen_profit = sum(network.links_t.p0["Unload_hydrogen"] * network.buses_t.marginal_price["Main_bus"] - network.links_t.p0["Load_hydrogen"] * network.buses_t.marginal_price["Main_bus"])
        
        print(self.battery_profit)
        print(self.hydrogen_profit)
        
        #calulate profiles of storages
        #battery
        self.battery_profile = -(network.links_t.p0["Load_battery"]-network.links_t.p0["Unload_battery"])/self.agents["Storage"]["data"].loc[self.timestep-1, "Battery_in"]
        
        #hydrogen
        self.hydrogen_profile = -(network.links_t.p0["Load_hydrogen"]-network.links_t.p0["Unload_hydrogen"])/self.agents["Storage"]["data"].loc[self.timestep-1, "Hydrogen_in"]
    ...
    def agent2(self):
        ...
        profit_outlook = sum(self.coal_profile*100) * (self.Electricity_cost_pure-self.agents["Producer 1"]["data"].loc[self.timestep, "Marginal_cost"])
    
        #ToDo: die profit berechnung muss geändert werden, nochmal herr nebel fragen
        ...
        #ToDo: Hier muss ich den interest rate einbauen. Ich könnte aus der interest rate einen factor errechnen um den der shift und slope verändert wird
        ...
        loans = {'Agent_name': ["Producer 1"], 'Active': [True], 'Runtime': [self.coal_lifetime], 'Amount': [amount], 'Interest_rate': [interest_rate], 'Payback': [payback]}
    ...
    def agent3(self):
        ...
        #investment function
        #calculate outlook of profit, multiply the electricity cost by the increased power and multiplie by hours in one year, assuming that the power plant runs the entire year and every mwh can be sold
        profit_outlook = sum(self.gas_ct_profile*50) * (self.Electricity_cost_pure-self.agents["Producer 2"]["data"].loc[self.timestep, "Marginal_cost"])
    ...
    def agent4(self):
        ...
        #investment function
        #calculate outlook of profit, multiply the electricity cost by the increased power and multiplie by hours in one year, assuming that the power plant runs the entire year and every mwh can be sold
        profit_outlook = sum(50*self.gas_cc_profile) * (self.Electricity_cost_pure-self.agents["Producer 3"]["data"].loc[self.timestep, "Marginal_cost"])
        ...
        #caluclate investment costs of this expansion over the lifetime
        investment_cost = ((50 * self.Capex.loc[self.timestep, "Gas CC"])/self.gas_lifetime)+self.Fix_costs.loc["Gas_CC"]*50
    ...
    def agent5(self):
        ...
        #update installed_power, generation decreased
        self.agents["Producer 4"]["data"].loc[self.timestep, "Installed_power"] = self.agents["Producer 4"]["data"].loc[self.timestep-1, "Installed_power"]*(1-self.nuclear_deprication)
    ...
    def agent6(self):
        ...
        #ToDo: extended power nicht mit profil berechnen sondern mit volllaststunden
    ...
    def agent11(self):
        ...
        #ToDo: Es wird zu wenig investiert!!!   
         #Villeicht liegt das nur an den sigmoid parametern
        ...
        print("\nBattery storage")
        #update capacity   
        self.agents["Storage"]["data"].loc[self.timestep, "Battery_cap"] = self.agents["Storage"]["data"].loc[self.timestep-1, "Battery_cap"]*0.933
        
        #update power in   
        self.agents["Storage"]["data"].loc[self.timestep, "Battery_in"] = self.agents["Storage"]["data"].loc[self.timestep-1, "Battery_in"]*0.933
        
        #update power out     
        self.agents["Storage"]["data"].loc[self.timestep, "Battery_out"] = self.agents["Storage"]["data"].loc[self.timestep-1, "Battery_out"]*0.933
        ...
        profit_outlook = sum((self.battery_profile*100)*self.marginal_costs)
        
        print("profit_outlook")
        print(profit_outlook)
        
        #calculat investment cost
        investment_cost = 100*self.Capex.loc[self.timestep, "Battery"]/self.battery_lifetime
        print("investment cost")
        print(investment_cost)
        
        #calculate profit margin
        profit_margin = (profit_outlook-investment_cost)/abs(profit_outlook)
        print("Profit margin")
        print(profit_margin)
        
        #calculate capacity_increase with sigmoid function
        #ToDo: werte zu variablen ändern
        capacity_increase = 5000*self.sigmoid(profit_margin, -3, -3, 0.25)
        print("capacity increase")
        print(capacity_increase)
        
        #update capacity
        self.agents["Storage"]["data"].loc[self.timestep, "Battery_cap"] = self.agents["Storage"]["data"].loc[self.timestep, "Battery_cap"] + capacity_increase
        
        #update power in   
        self.agents["Storage"]["data"].loc[self.timestep, "Battery_in"] = self.agents["Storage"]["data"].loc[self.timestep, "Battery_in"]+ capacity_increase/2
        
        #update power out     
        self.agents["Storage"]["data"].loc[self.timestep, "Battery_out"] = self.agents["Storage"]["data"].loc[self.timestep, "Battery_out"]+ capacity_increase/2
        
        
        #Quotient kapazität zu leistung 50% -> leistung ist immer 50% von der kapazität
        ...
        print("\nHydrogen storage")
        #update capacity   
        self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_cap"] = self.agents["Storage"]["data"].loc[self.timestep-1, "Hydrogen_cap"] * 0.96
        
        #update power in   
        self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_in"] = self.agents["Storage"]["data"].loc[self.timestep-1, "Hydrogen_in"] * 0.96
        
        #update power out     
        self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_out"] = self.agents["Storage"]["data"].loc[self.timestep-1, "Hydrogen_out"] * 0.96
        
        #update efficiency
        self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_eff"] = self.agents["Storage"]["data"].loc[self.timestep-1, "Hydrogen_eff"]
        
        #calculate profit outlook
        profit_outlook = sum((self.hydrogen_profile*100)*self.marginal_costs)
        print("profit_outlook")
        print(profit_outlook)
        
        #calculat investment cost
        investment_cost = 100*self.Capex.loc[self.timestep, "Hydrogen"]/self.hydrogen_lifetime
        print("investment cost")
        print(investment_cost)
        ...
        #update capacity   
        self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_cap"] = self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_cap"] + capacity_increase
        
        #update power in   
        self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_in"] = self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_in"] + capacity_increase/5
        
        #update power out     
        self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_out"] = self.agents["Storage"]["data"].loc[self.timestep, "Hydrogen_out"] + capacity_increase/5
        
        
        #Quotient kapazität zu leistung 50% -> leistung ist immer 20% von der kapazität      
...
if __name__ == '__main__':
    ...
    #Generated Power
    ess.agents["Producer 1"]["data"]["Generated_power_total"][0:end_year-start_year].plot(title="Generated Power")
    ess.agents["Producer 2"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    ess.agents["Producer 3"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    ess.agents["Producer 4"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    ess.agents["Producer 5"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    ess.agents["Producer 6"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    ess.agents["Producer 7"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    ess.agents["Producer 8"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    ess.agents["Producer 9"]["data"]["Generated_power_total"][0:end_year-start_year].plot()
    plt.show()
    
    #installed power
    ess.agents["Producer 1"]["data"]["Installed_power"][0:end_year-start_year].plot(title="Installed Power")
    ess.agents["Producer 2"]["data"]["Installed_power"][0:end_year-start_year].plot()
    ess.agents["Producer 3"]["data"]["Installed_power"][0:end_year-start_year].plot()
    ess.agents["Producer 4"]["data"]["Installed_power"][0:end_year-start_year].plot()
    ess.agents["Producer 5"]["data"]["Installed_power"][0:end_year-start_year].plot()
    ess.agents["Producer 6"]["data"]["Installed_power"][0:end_year-start_year].plot()
    ess.agents["Producer 7"]["data"]["Installed_power"][0:end_year-start_year].plot()
    ess.agents["Producer 8"]["data"]["Installed_power"][0:end_year-start_year].plot()
    ess.agents["Producer 9"]["data"]["Installed_power"][0:end_year-start_year].plot()
    plt.show()
    
    #Load
    ess.agents["Producer 1"]["data"]["Load"][1:end_year-start_year].plot(title="Load")
    ess.agents["Producer 2"]["data"]["Load"][1:end_year-start_year].plot()
    ess.agents["Producer 3"]["data"]["Load"][1:end_year-start_year].plot()
    ess.agents["Producer 4"]["data"]["Load"][1:end_year-start_year].plot()
    ess.agents["Producer 5"]["data"]["Load"][1:end_year-start_year].plot()
    ess.agents["Producer 6"]["data"]["Load"][1:end_year-start_year].plot()
    ess.agents["Producer 7"]["data"]["Load"][1:end_year-start_year].plot()
    ess.agents["Producer 8"]["data"]["Load"][1:end_year-start_year].plot()
    ess.agents["Producer 9"]["data"]["Load"][1:end_year-start_year].plot()
    plt.show()
    ...
    #Marginal costs
    ess.agents["Producer 1"]["data"]["Marginal_cost"][0:end_year-start_year].plot(title="Marginal costs")
    ess.agents["Producer 2"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    ess.agents["Producer 3"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    ess.agents["Producer 4"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    ess.agents["Producer 5"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    ess.agents["Producer 6"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    ess.agents["Producer 7"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    ess.agents["Producer 8"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    ess.agents["Producer 9"]["data"]["Marginal_cost"][0:end_year-start_year].plot()
    plt.show()
    
    #Total_emissions
    #ess.agents["Producer 1"]["data"]["Total_emissions"][2:end_year-start_year].plot(title="Emissions")
    #ess.agents["Producer 2"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Producer 3"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Producer 4"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Producer 5"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Producer 6"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Producer 7"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Producer 8"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Producer 9"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    #ess.agents["Government"]["data"]["Total_emissions"][2:end_year-start_year].plot()
    plt.show()
    
    #Money
    ess.agents["Producer 1"]["data"]["Money"][1:end_year-start_year].plot(title="Money")
    ess.agents["Producer 2"]["data"]["Money"][1:end_year-start_year].plot()
    ess.agents["Producer 3"]["data"]["Money"][1:end_year-start_year].plot()
    ess.agents["Producer 4"]["data"]["Money"][1:end_year-start_year].plot()
    ess.agents["Producer 5"]["data"]["Money"][1:end_year-start_year].plot()
    ess.agents["Producer 6"]["data"]["Money"][1:end_year-start_year].plot()
    ess.agents["Producer 7"]["data"]["Money"][1:end_year-start_year].plot()
    ess.agents["Producer 8"]["data"]["Money"][1:end_year-start_year].plot()
    ess.agents["Producer 9"]["data"]["Money"][1:end_year-start_year].plot()
    plt.show()
    
    #Income
    ess.agents["Producer 1"]["data"]["Income"][1:end_year-start_year].plot(title="Income")
    ess.agents["Producer 2"]["data"]["Income"][1:end_year-start_year].plot()
    ess.agents["Producer 3"]["data"]["Income"][1:end_year-start_year].plot()
    ess.agents["Producer 4"]["data"]["Income"][1:end_year-start_year].plot()
    ess.agents["Producer 5"]["data"]["Income"][1:end_year-start_year].plot()
    ess.agents["Producer 6"]["data"]["Income"][1:end_year-start_year].plot()
    ess.agents["Producer 7"]["data"]["Income"][1:end_year-start_year].plot()
    ess.agents["Producer 8"]["data"]["Income"][1:end_year-start_year].plot()
    ess.agents["Producer 9"]["data"]["Income"][1:end_year-start_year].plot()
    plt.show()
    
    
    #Power demand
    #ess.agents["Consumer 1"]["data"]["Power_demand"][2:end_year-start_year].plot(title="Power demand")
    #ess.agents["Consumer 2"]["data"]["Power_demand"][2:end_year-start_year].plot()
    #ess.agents["Consumer 3"]["data"]["Power_demand"][2:end_year-start_year].plot()
    #ess.agents["Government"]["data"]["Total_demanded_power"][2:end_year-start_year].plot()
    plt.show()
    
    #electricity cost
    ess.agents["Government"]["data"]["Electricity_cost"][0:end_year-start_year].plot(title="Electricity price")
    plt.show()
    ...
    #LIBOR
    ess.agents["Bank"]["data"]["LIBOR_index"][2:end_year-start_year].plot(title="LIBOR index")
    plt.show()
    ...
    Producer_Coal=ess.agents["Producer 1"]["data"]
    Producer_Gas_CT=ess.agents["Producer 2"]["data"]
    Producer_Gas_CC=ess.agents["Producer 3"]["data"]
    Producer_Nuclear=ess.agents["Producer 4"]["data"]
    Producer_Solar=ess.agents["Producer 5"]["data"]
    Producer_Wind=ess.agents["Producer 6"]["data"]
    Producer_Hydro=ess.agents["Producer 7"]["data"]
    Producer_Biomass=ess.agents["Producer 8"]["data"]
    Producer_Oil=ess.agents["Producer 9"]["data"]
