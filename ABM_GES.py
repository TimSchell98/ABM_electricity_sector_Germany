...
class ESS:
    def __init__(self, start_year=2000, end_year=2020, governmental_funding = 5, battery_store_cap = 50000, battery_store_in_power = 2500,
                 ...
                 coal_slope = -8, coal_shift = -0.45, coal_max = 3500,
                 gas_ct_slope = -9, gas_ct_shift = -0.5, gas_ct_max = 1000,
                 gas_cc_slope = -9, gas_cc_shift = -0.5, gas_cc_max = 2000,
                 solar_slope = -4, solar_shift = 0, solar_max = 9000,
                 wind_slope = -4, wind_shift = 0, wind_max = 6000,
    ...
    def loan_list(self):
        ...
        for i in range(1, len(self.loans)):
            ...
            #print(self.loans.iloc[i,0], self.loans.iloc[i,3])
            
            #check if loan is still active
            if self.loans.iloc[i,1] == True:
                
                #substract payback from income of agent, check if this is right
    ...
    def agent2(self):
        ...
        self.agents["Producer 1"]["data"].loc[self.timestep, "Installed_power"] = self.agents["Producer 1"]["data"].loc[self.timestep-1, "Installed_power"] * 0.97
        ...
        self.agents["Producer 1"]["data"].loc[self.timestep, "Income"] = self.agents["Producer 1"]["data"].loc[self.timestep, "Generated_power_total"]*(self.Electricity_cost_pure-self.agents["Producer 1"]["data"].loc[self.timestep, "Marginal_cost"])

        #update money
        self.agents["Producer 1"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 1"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 1"]["data"].loc[self.timestep, "Income"] 
        ...
        #ToDo: hier muss ich noch die marginalen costen abziehen -> die agenten machen dann aber unfassbar wenig gewinn
        ...
        #caluclate investment costs of this expansion over a lifetime of 40 years
        investment_cost = (100 * self.Capex.loc[self.timestep, "Coal"])/40
        ...
        proportion_delta = (profit_outlook-investment_cost)/profit_outlook
        ...
        """
        if proportion_delta >= -self.kohle_verschiebung:
            #calculate power_increase in 100 MW steps
            power_increase = self.kohle_steigung*(proportion_delta+self.kohle_verschiebung)**2
            #converte to MW
            power_increase = round(power_increase*100,4)
      
            #check if power increase is to high
            if power_increase > self.kohle_max:
                power_increase = self.kohle_max  
    ...
    def agent3(self):
        ...
        self.agents["Producer 2"]["data"].loc[self.timestep, "Installed_power"] = self.agents["Producer 2"]["data"].loc[self.timestep-1, "Installed_power"] * 0.967
        ...
        #calculate Income with calculated PyPSA data
        self.agents["Producer 2"]["data"].loc[self.timestep, "Income"] = self.agents["Producer 2"]["data"].loc[self.timestep, "Generated_power_total"]*(self.Electricity_cost_pure-self.agents["Producer 2"]["data"].loc[self.timestep, "Marginal_cost"])

        #update money
        self.agents["Producer 2"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 2"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 2"]["data"].loc[self.timestep, "Income"] 
        ...
        investment_cost = (50 * self.Capex.loc[self.timestep, "Gas CT"])/30
    ...
    def agent4(self):
        ...
        #update installed_power, decrease because of deprication 3.33% per year -> lifetime 30 years
        self.agents["Producer 3"]["data"].loc[self.timestep, "Installed_power"] = self.agents["Producer 3"]["data"].loc[self.timestep-1, "Installed_power"] * 0.967
        ...
        #calculate Income with calculated PyPSA data
        self.agents["Producer 3"]["data"].loc[self.timestep, "Income"] = self.agents["Producer 3"]["data"].loc[self.timestep, "Generated_power_total"]*(self.Electricity_cost_pure-self.agents["Producer 3"]["data"].loc[self.timestep, "Marginal_cost"])

        #update money
        self.agents["Producer 3"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 3"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 3"]["data"].loc[self.timestep, "Income"] 

        #investment function
        #calculate theoratical energy to calculate load of producer, installed power from the last timestep, because in PyPSA also the previous installed power is used
        theoratical_energy_total = self.agents["Producer 3"]["data"].loc[self.timestep, "Installed_power"] * self.annual_full_load_hours_gas
        ...
        #caluclate investment costs of this expansion over a lifetime of 30 years
        investment_cost = (100 * self.Capex.loc[self.timestep, "Gas CC"])/30
    ...
    def agent5(self):
        ...
        #calculate Income with calculated PyPSA data
        self.agents["Producer 4"]["data"].loc[self.timestep, "Income"] = self.agents["Producer 4"]["data"].loc[self.timestep, "Generated_power_total"]*self.Electricity_cost_pure

        #update money
        self.agents["Producer 4"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 4"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 4"]["data"].loc[self.timestep, "Income"] 
    ...
    def agent6(self):
        ...
        self.agents["Producer 5"]["data"].loc[self.timestep, "Installed_power"] = self.agents["Producer 5"]["data"].loc[self.timestep-1, "Installed_power"]* 0.96

        #fill generated_power_total with calculated PyPSA data
        self.agents["Producer 5"]["data"].loc[self.timestep, "Generated_power_total"] = self.generated_power.iloc[self.timestep,4]

        #calculate Income with calculated PyPSA data
        self.agents["Producer 5"]["data"].loc[self.timestep, "Income"] = self.agents["Producer 5"]["data"].loc[self.timestep, "Generated_power_total"]*self.Electricity_cost_pure

        #update money
        self.agents["Producer 5"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 5"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 5"]["data"].loc[self.timestep, "Income"] 
        ...
        #caluclate investment costs of this expansion over a lifetime of 20 years -> ToDo: Hier ist noch kein zinz drauf und vorallem keine tilgung! -> muss hier überhaupt n Zinz/tilgung drauf?
        investment_cost = ((self.agents["Producer 5"]["data"].loc[self.timestep, "Installed_power"] * 0.05)* self.Capex.loc[self.timestep, "Solar Utility"])/25
    ...
    def agent7(self):
        ...
        self.agents["Producer 6"]["data"].loc[self.timestep, "Installed_power"] = self.agents["Producer 6"]["data"].loc[self.timestep-1, "Installed_power"] * 0.96
        ...
        #calculate Income with calculated PyPSA data
        self.agents["Producer 6"]["data"].loc[self.timestep, "Income"] = self.agents["Producer 6"]["data"].loc[self.timestep, "Generated_power_total"]* (self.Electricity_cost_pure + self.Governmental_funding)

        #update money
        self.agents["Producer 6"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 6"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 6"]["data"].loc[self.timestep, "Income"] 
        ...
        profit_outlook = extended_power_total * (self.Electricity_cost_pure+self.Governmental_funding)
        #caluclate investment costs of this expansion over a lifetime of 20 years -> ToDo: Hier ist noch kein zinz drauf und vorallem keine tilgung! -> muss hier überhaupt n Zinz/tilgung drauf?
        investment_cost = ((self.agents["Producer 6"]["data"].loc[self.timestep, "Installed_power"] * 0.05)* self.Capex.loc[self.timestep, "Wind Onshore"])/25
    ...
    def agent8(self):
        ...
        #update money
        self.agents["Producer 7"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 7"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 7"]["data"].loc[self.timestep, "Income"] 
    ...
    def agent9(self):
        ...
        #update money
        self.agents["Producer 8"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 8"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 8"]["data"].loc[self.timestep, "Income"] 
    ...
    def agent10(self):
        ...
        #update money
        self.agents["Producer 9"]["data"].loc[self.timestep, "Money"] = self.agents["Producer 9"]["data"].loc[self.timestep-1, "Money"] + self.agents["Producer 9"]["data"].loc[self.timestep, "Income"] 
